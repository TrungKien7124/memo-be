    ...
    def _create_course(self, teacher):
        ...
        for i, (mod_title, lessons) in enumerate([
            ('Greetings & Introductions', [
                ...
                ('Introducing Yourself', 'https://www.youtube.com/watch?v=example2'),
            ]),
            ('Daily Conversations', [
                ('At the Restaurant', 'https://www.youtube.com/watch?v=example3'),
                ('Shopping', 'https://www.youtube.com/watch?v=example4'),
                ('Asking for Directions', 'https://www.youtube.com/watch?v=example5'),
            ]),
        ]):
            module = Module.objects.create(course=course, title=mod_title, order_index=i)
            for j, (lesson_title, video_url) in enumerate(lessons):
                Lesson.objects.create(
                    module=module, title=lesson_title,
                    video_url=video_url, order_index=j,
                )
    ...
    def _create_flashcards(self, student):
        ...
        for front, back, ipa in cards:
            Flashcard.objects.create(
                user=student, folder=folder,
                front_text=front, back_text=back, ipa=ipa,
            )
